...
print("[lora] run_overfitting.py imported")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LoRA training with repeated epochs.")
    parser.add_argument(
        "--tier",
...
def main():
    ...
    if not args.quiet:
        ...
        print(
            "[lora] hyperparams: "
            f"epochs={args.epochs} repeat={args.repeat} "
    ...
    for repeat_idx in range(args.repeat):
        epoch_target = args.epochs + repeat_idx
        print(
            f"[lora] repeat {repeat_idx + 1}/{args.repeat} "
            f"(epochs={epoch_target})"
        )
        tuner = FineTuner(
        ...
        if repeat_idx + 1 < args.repeat:
            print("[lora] sleeping 1s before next repeat...")
            import time
            time.sleep(1)
